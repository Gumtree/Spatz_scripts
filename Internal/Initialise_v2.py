...
from org.gumtree.control.ui.batch import BatchManagerListenerAdapter
from org.gumtree.control.ui.batch import BatchManager
...
__scan_status_node__ = sics.get_controller('/commands/scan/runscan/feedback/status')
__scan_variable_node__ = sics.get_controller('/commands/scan/runscan/scan_variable')
__save_count_node__ = sics.get_controller('/experiment/save_count')
__file_name_node__ = sics.get_controller('/experiment/file_name')
__file_status_node__ = sics.get_controller('/experiment/file_status')
...
class BatchStatusListener(BatchManagerListenerAdapter):
    
    def __init__(self):
        pass
    
    def statusChanged(self, newStatus, message):
        logBook('Batch status = ' + str(newStatus))
        logBook(str(message))

    def outputReceived(self, outputText):
        logBook(str(outputText))

    def scriptFinished(self, replyText):
        logBook(str(replyText))

_batchManager = BatchManager.getBatchScriptManager(sics.proxy)
try:
    _batchManager.removeBatchManagerListener(__batch_status_listener__)
except:
    pass
__batch_status_listener__ = BatchStatusListener()
_batchManager.addBatchManagerListener(__batch_status_listener__)
...
def __dispose_all__(event):
    ...
    global _batchManager
    if _batchManager != None:
        _batchManager.removeBatchManagerListener(__batch_status_listener__)
